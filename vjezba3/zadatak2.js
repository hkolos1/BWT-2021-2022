...
http.createServer(function (request, response) {
    ...
    if(request.method=='GET'){
        fs.readFile('imenik.txt', function (error, contents){
            if(error)
                throw error;
            let pretragaParametar = new url.URLSearchParams(request.url);
            var q = pretragaParametar.get('/?q');
            let ispisanElement = false;
            let prviElement = true;
            ...
            for (let i = 0; i < nizElemenata.length-1; i++) {
                ...
                ispisanElement = true; prviElement = false;
            }
            if(!ispisanElement)
